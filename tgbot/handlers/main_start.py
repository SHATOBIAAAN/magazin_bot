...
async def main_start(message: Message, state: FSMContext):
    ...
    if s['is_ref'] == 'True':
        if message.get_args() == "":
            ...
            ref_user = await db.get_user(id=int(message.get_args()))
            if ref_user is None:
                await send_start_message()
            else:
                if user['ref_id'] is not None:
                    ...
                    if ref_user['id'] == message.from_user.id:
                        await message.answer(lang.invite_yourself)
                    else:
                        user_ref_count = ref_user['ref_count']  # Исправлено: обращение к значению по ключу
                        ...
                        await db.update_user(message.from_user.id, ref_id=ref_user['id'],
                                             ref_user_name=ref_user['user_name'], ref_first_name=ref_user['first_name'])
                        await db.update_user(ref_user['id'], ref_count=user_ref_count + 1)

                        await bot.send_message(chat_id=ref_user['id'], text=msg)

                        if ref_user['ref_count'] + 1 == s['ref_lvl_1']:
                            remain_refs = s['ref_lvl_2'] - (ref_user['ref_count'] + 1)
                            text = lang.new_ref_lvl.format(new_lvl=1, next_lvl=2, remain_refs=remain_refs,
                                                           convert_ref=convert_ref(lang, remain_refs))
                            await bot.send_message(chat_id=ref_user['id'], text=text)
                            await db.update_user(ref_user['id'], ref_lvl=1)
                        elif ref_user['ref_count'] + 1 == s['ref_lvl_2']:
                            remain_refs = s['ref_lvl_3'] - (ref_user['ref_count'] + 1)
                            text = lang.new_ref_lvl.format(new_lvl=2, next_lvl=3, remain_refs=remain_refs,
                                                           convert_ref=convert_ref(lang, remain_refs))
                            await bot.send_message(chat_id=ref_user['id'], text=text)
                            await db.update_user(ref_user['id'], ref_lvl=2)
                        elif ref_user['ref_count'] + 1 == s['ref_lvl_3']:
                            await bot.send_message(chat_id=ref_user['id'], text=lang.max_ref_lvl)
                            await db.update_user(ref_user['id'], ref_lvl=3)
